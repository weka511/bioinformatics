...
class HTreeBuilder(Tree):
    ...
    def build(self,T):
        ...
        return Tree.from_clade(self.get_root())
...
def Count(v,species,sizes=[]):
    ...
    if v.is_terminal():
        ...
        y = Count(get_subtree(v, small=False), sizes=sizes)
        colour_leaves(get_subtree(v), Colour.A)
        z = Count(get_subtree(v), sizes=sizes)
        return x + y + z

def decorate(HT,S):
    for clade in HT.find_elements(order='postorder'):
